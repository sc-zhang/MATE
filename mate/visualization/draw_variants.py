...
def __draw_genes_in_association(cleanup_aln_dir, asc_file, pic_dir, pic_fmt):
    ...
    for gene in var_sites_db:
        ...
        seq_set = set()
        seq_len = 0
        pic_file = path.join(pic_dir, "%s.%s" % (gene, pic_fmt))
...
def draw_variant_sites_in_association(cleanup_aln_dir, asc_dir, pic_dir, pic_fmt, thread):
    ...
    for fn in listdir(asc_dir):
        ...
        res.append(
            [fn, pool.apply_async(__draw_genes_in_association, (cleanup_aln_dir, asc_file, sub_pic_dir, pic_fmt,))])
    pool.close()
    pool.join()
