...
options:
  ...
  -q, --query QUERY     Directory contain all query files
  --query_type {genome,bam,cds}
                        Type of query file, could be genome/bam/cds, default=genome
  -r, --reference REFERENCE
                        Reference file of candidate genes
  --ref_type {cds,gff3}
                        Type of reference file, could be cds/gff3, when query_type is genome, it only could be set to cds, default=cds
  ...
  --cds_align           If set this parameter, the multi sequences alignment would be applied on cds, otherwise it would be converted to pep first
  --variant_filter VARIANT_FILTER
                        Threshold string for variant caller, "kmer_length:lower_threshold:missing_threshold", kmer_length means the size of kmer; lower_threshold means if one position contain a kmer with less thanthis ratio of samples support, drop it; missing_threshold means for one position if more than this
                        ratio of samples are "-" at drop it; default=5:0.05:0.9
  --allele_filter ALLELE_FILTER
                        ...
                        samples supported, it would be dropped; missing_threshold means if one gene with more than this ratio of samples marked as absence, it would be dropped; min_allele means if one gene with less than this count of alleles (ignore absence), it would be dropped; default=0:1:1:1 (no filter)
...
**Notice** if query_type is bam and ref_type is cds, the bam files should be generated by mapping reads to cds files,
if query_type is bam and ref_type is gff3, the bam files should be generated by mapping reads to reference genome.  
